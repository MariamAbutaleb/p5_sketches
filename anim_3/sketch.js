...
function draw() {
	...
	for (x = 0; x < 200; x++) {
		console.log(x);
		for (y = 0; y < 112; y++) {
			var redIn = inputImage.pixels[(y * 400 * frameCount + x) * 4 + 0];
			var greenIn = inputImage.pixels[(y * 400 * frameCount + x) * 4 + 1];
			var blueIn = inputImage.pixels[(y * 400 * frameCount + x) * 4 + 2];

			var redOut, greenOut, blueOut;

			inputImage.pixels[(y * 400 * frameCount + x) * 4 + 0] = redOut;
			inputImage.pixels[(y * 400 * frameCount + x) * 4 + 1] = greenOut;
