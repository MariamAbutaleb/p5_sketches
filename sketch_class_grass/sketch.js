...
function preload() {
	inputImage = loadImage("assets/cf.png");

	heightSlider = createSlider(0, 100, 50);
	heightSlider.changed(draw); //because noLopp is activated, whenever the heightSlider value changes, it redraws the entire thing
}
...
function draw() {
	...
	for (y = 0; y < 320; y++) {
		for (x = 0; x < 640; x++) {
			...
			if (random(255) < red) { //compare to image to decide to draw
				if (random(100) > 95) {
					drawGrass(x, y);
				}
			}
